...
import { observeDgReportTransportPayload } from "./dg-report-sync";

export type DgRoadResult = "莊" | "閒" | "和";
export type DgBeadCell = {
  result: DgRoadResult;
  bankerPair: boolean;
  playerPair: boolean;
  lucky6: boolean;
...
function roadCell(raw: string): DgBeadCell | null {
  const parts = String(raw || "").split("#");
  const code = Number(parts[1] ?? parts[0]);
  if (!Number.isInteger(code) || code < 1 || code > 12) return null;

  // DG baccarat road code: 1~4 banker, 5~8 player, 9~12 tie.
  // Within each block: +0 no pair, +1 banker pair, +2 player pair, +3 both.
  // road[2] is the banker-six flag; road[3] is the winning-side point.
  const base = (code - 1) % 4;
  const result: DgRoadResult = code <= 4 ? "莊" : code <= 8 ? "閒" : "和";
  const pointRaw = Number(parts[3]);
  return {
    result,
    bankerPair: base === 1 || base === 3,
    playerPair: base === 2 || base === 3,
    lucky6: Number(parts[2]) === 1,
    point: Number.isFinite(pointRaw) ? pointRaw : null,
    rawCode: code,
  };
}
function parseBeads(roads: string[] | undefined) {
  // DG PublicBean.Table.roads / cmd=1004 list is newest -> oldest.
  // 珠盤按時間順序由上往下填滿 6 格，再往右一欄。
  return [...(roads || [])].reverse().map(roadCell).filter((x): x is DgBeadCell => !!x);
}
function parseRoads(roads: string[] | undefined) {
  return parseBeads(roads).map(x => x.result);
}
...
// DG's protocol uses internal FMS ids such as BAC001 / TID348, while the
// visible lobby and the floating assistant use room codes such as RB01 / S01.
// Always normalize to the visible room code so SSE table keys match the UI.
...
function roomCode(raw: any, prev: DgTableSnapshot | undefined, tableId: number) {
  const values = [raw?.tableName, prev?.apiId, ROOM_BY_TABLE.get(tableId), raw?.fms];
  for (const value of values) {
    const code = String(value ?? "").trim().toUpperCase();
    if (/^RB0[1-5]$/.test(code) || /^S(?:0[1-7]|09|10)$/.test(code)) return code;
  }
  return ROOM_BY_TABLE.get(tableId) || String(raw?.tableName ?? raw?.fms ?? `DG${tableId}`).trim().toUpperCase();
}


function maybeJsonText(text: string): unknown {
...
function printableUtf8(part: Buffer) {
  if (!part.length || part.length > 2_000_000) return '';
  const text = part.toString('utf8').replace(/\u0000/g, '').trim();
  if (!text || text.includes('\uFFFD')) return '';
  let printable = 0;
  for (const ch of text) if (ch === '\n' || ch === '\r' || ch === '\t' || ch.charCodeAt(0) >= 0x20) printable++;
  return printable / Math.max(text.length, 1) >= 0.9 ? text : '';
}

function extractTransportStrings(buf: Buffer, maxDepth = 5) {
  const out: string[] = [];
  const seen = new Set<string>();
  const walk = (part: Buffer, depth: number) => {
    if (!part.length || depth > maxDepth) return;
    let o = 0, fields = 0;
    try {
      while (o < part.length && fields++ < 512) {
        const key = readVarint(part, o); o = key.offset;
        const wire = Number(key.value & 7n);
        if (wire === 0) { o = readVarint(part, o).offset; continue; }
        if (wire === 1) { o += 8; continue; }
        if (wire === 5) { o += 4; continue; }
        if (wire !== 2) return;
        const seg = readLength(part, o); o = seg.offset;
        const child = part.subarray(seg.start, seg.end);
        const text = printableUtf8(child);
        if (text && text.length <= 200_000 && !seen.has(text)) { seen.add(text); out.push(text); }
        if (child.length >= 2 && child.length <= 1_000_000) walk(child, depth + 1);
      }
    } catch {}
  };
  walk(buf, 0);
...
class RawWsClient {
  ...
  connect() {
    ...
    // Road/live packets are tiny and time-sensitive. Disable Nagle so a fresh
    // baccarat result is not held waiting for another TCP packet.
    ...
    sock.once("secureConnect", () => {
      const req = [
        ...
        // Do not negotiate permessage-deflate. RawWsClient intentionally parses
        // uncompressed binary protobuf frames and does not implement RSV1/zlib.
        "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/135.0.0.0 Safari/537.36", "", ""
      ].join("\r\n");
      sock.write(req);
    });
...
export class DgRelay {
  ...
  private lastDecodedPacketAt = 0;
  private lastTablePacketAt = 0;
  private lastSnapshotRequestAt = 0;
  private lastSubscriptionRecoveryAt = 0;
  private feedStale = false;
  private recentForegroundRequests: Array<{ at: number; cmd: number; data: Buffer }> = [];
  private reportReplayFrame: Buffer | null = null;
  private reportReplayFrames: Array<{cmd:number;data:Buffer}> = [];
  private reportReplayCmd = 0;
  private lastReportRefreshAt = 0;
  ...
  getResolvedLaunchUrl(): string { return this.launchUrl || this.gameUrl; }
  getHealth() {
    return {
      state: this.status,
      message: this.statusMessage,
      lastPacketAt: this.lastVendorPacketAt || null,
      lastDecodedPacketAt: this.lastDecodedPacketAt || null,
      lastTablePacketAt: this.lastTablePacketAt || null,
      tables: this.map.size,
  ...
  private open() {
    ...
    this.ws = new RawWsClient(url, this.origin, data => {
      // Foreground DG must see the exact vendor packet first. Previously the
      // assistant decoded/broadcast the packet before forwarding it to the DG
      // page, so under Render CPU load the floating panel could already show
      // the new hand while the vendor bead plate was still one step behind.
      // Enqueue the raw frame to every DG browser socket first, then do our
      // protobuf/SSE/report work. Both views still share this ONE upstream WS.
  ...
  requestFreshSnapshot(reason = "auto") {
    if (this.stopped || this.transportMode !== "raw" || this.status !== "connected") return false;
    const now = Date.now();
    if (now - this.lastSnapshotRequestAt < 2500) return false;
    this.lastSnapshotRequestAt = now;
    // Refresh the existing authenticated socket only. No new DG login/token is created.
    this.send(2, { lobbyId: 5, type: 0 });
    this.send(87, { type: 1 });
    this.send(24, { type: 2 });
    if (reason !== "timer") this.log(`已要求最新牌路快照（${reason}）`);
    return true;
  }

  private requestVideos() {
    if (this.transportMode !== "raw") return;
    const baccarat = this.tables().filter(t => /^RB0[1-5]$/i.test(t.apiId) || /^S(?:0[1-7]|09|10)$/i.test(t.apiId) || /^BAC\d+/i.test(t.apiId) || /^TID\d+/i.test(t.apiId));
  ...
  private inspectReportTransport(bean: PublicBean, data: Buffer, cmd: number) {
    const strings = extractTransportStrings(data);
    const payload = {
      cmd,
      gameNo: bean.gameNo,
      tableId: bean.tableId,
      userName: bean.userName,
      object: bean.object ? maybeJsonText(bean.object) : undefined,
      list: (bean.list || []).map(maybeJsonText),
      strings: strings.map(maybeJsonText),
    };
    let accepted = false;
    try { accepted = observeDgReportTransportPayload(this.sessionId, payload, `dg-ws-cmd-${cmd}`); } catch {}
    if (accepted) {
      const now = Date.now();
      const candidates = this.recentForegroundRequests.filter(x => now - x.at <= 10_000);
      const sameCmd = [...candidates].reverse().find(x => x.cmd === cmd);
      const req = sameCmd || candidates[candidates.length - 1];
      if (req) {
        this.reportReplayFrame = Buffer.from(req.data);
        this.reportReplayCmd = req.cmd;
        this.captureRecentReportRequests(10_000);
        this.log(`已鎖定 DG 報表即時通道 cmd=${cmd}｜刷新請求 cmd=${req.cmd}`);
      } else {
        this.log(`已讀到 DG 報表即時資料 cmd=${cmd}`);
      }
    }
    return accepted;
  }

  captureRecentReportRequests(windowMs = 6000) {
    const now = Date.now();
    const recent = this.recentForegroundRequests.filter(x => now - x.at <= windowMs);
    if (!recent.length) return false;
    const picked: Array<{cmd:number;data:Buffer}> = [];
    const seen = new Set<string>();
    for (const item of recent.slice(-6)) {
      const sig = `${item.cmd}:${item.data.subarray(0,48).toString('base64')}`;
      if (seen.has(sig)) continue;
      seen.add(sig); picked.push({cmd:item.cmd,data:Buffer.from(item.data)});
    }
    if (!picked.length) return false;
    this.reportReplayFrames = picked.slice(-4);
    const last = this.reportReplayFrames[this.reportReplayFrames.length - 1]!;
    this.reportReplayFrame = Buffer.from(last.data);
    this.reportReplayCmd = last.cmd;
    this.log(`已記住本玩家 DG 報表刷新請求：${this.reportReplayFrames.map(x=>x.cmd).join(',')}`);
    return true;
  }

  requestReportRefresh(force = false) {
    if (!this.ws || this.status !== "connected") return false;
    const frames = this.reportReplayFrames.length ? this.reportReplayFrames : (this.reportReplayFrame ? [{cmd:this.reportReplayCmd,data:this.reportReplayFrame}] : []);
    if (!frames.length) return false;
    const now = Date.now();
    if (!force && now - this.lastReportRefreshAt < 2500) return false;
    this.lastReportRefreshAt = now;
    let any = false;
    try {
      for (const frame of frames) any = this.ws.sendBinary(Buffer.from(frame.data)) || any;
      if (any && force) this.log(`已重送本玩家 DG 報表刷新請求：${frames.map(x=>x.cmd).join(',')}`);
      return any;
    } catch { return false; }
  }
  ...
  private handle(data: Buffer) {
    ...
    this.lastDecodedPacketAt = Date.now();
    const cmd = n(bean.cmd);
    this.inspectReportTransport(bean, data, cmd);
    const isTablePayload = cmd === 1004 || !!bean.table?.length;
    if (this.feedStale && isTablePayload) {
    ...
    if (cmd === 10086) {
      ...
      if (this.transportMode === "raw") {
        ...
        // DG 原生頁面以 cmd=99 維持連線。保留同一個已驗證 socket，
        // 但「沒有新牌局」不代表斷線；前一版每 30 秒依 table silence
        // 重建 socket，會讓正在玩的 DG 畫面反覆載入。
        this.lastDecodedPacketAt = Date.now();
        this.lastTablePacketAt = Date.now();
        this.lastVendorPacketAt = Date.now();
        ...
        this.keepaliveTimer = setInterval(() => {
          ...
          this.send(99);
          const now = Date.now();
          const vendorSilence = now - this.lastVendorPacketAt;
          const tableSilence = now - this.lastTablePacketAt;

          // 遊戲畫面已接到 shared socket 時，讓 DG 自己控制訂閱狀態。
          // 不再每 6/12 秒塞 lobby snapshot / subscribe 指令進正在玩的桌，
          // 避免影片區與桌面反覆重新初始化。只有上游完全沒有任何封包
          // 很久時，才用同一 token 重建 upstream；不會重新 DGLI/login。
          if (this.foregroundBridgeActive) {
            if (vendorSilence > 45000) {
              this.log(`DG 上游 ${Math.round(vendorSilence/1000)} 秒無任何封包，重建同一 token 的即時連線`);
              this.ws?.abort("DG vendor feed silent");
            }
            return;
          }

          if (vendorSilence > 30000) {
            this.log(`DG 上游 ${Math.round(vendorSilence/1000)} 秒無任何封包，重建同一 token 的即時連線`);
            this.ws?.abort("DG vendor feed silent");
            return;
          }

          // 沒有開新局時只在原 socket 上低頻重送訂閱，不因 table silence
          // 強制斷線，也不建立第二個登入工作階段。
          if (tableSilence > 45000 && now - this.lastSubscriptionRecoveryAt > 45000) {
            ...
            this.send(87, { type: 1 });
            this.send(24, { type: 2 });
            this.log("牌路 45 秒無變化，已在原連線重送訂閱（未重新登入、未斷線）");
          }
        }, 3000);
      }
    }
    ...
    if (cmd === 1004 && bean.tableId && bean.list?.length) {
      ...
      if (!prev) {
        ...
        const beads = parseBeads(fullRoads);
        const results = beads.map(x => x.result);
        const counts = countResults(results);
        this.map.set(tableId, { ...prev, results, beads, ...counts, lastUpdated: Date.now(), lastResultKey: `${prev.shoe}:${prev.round}:${results.length}:${results.at(-1) || ""}` });
    ...
    if (bean.table?.length) {
      ...
      for (const raw of bean.table) {
        ...
        const roads = queuedRoads && (!snapshotRoads || queuedRoads.length >= snapshotRoads.length)
          ? queuedRoads
          : snapshotRoads;
        const beads = roads ? parseBeads(roads) : (prev?.beads ?? []);
        const results = beads.length ? beads.map(x => x.result) : (prev?.results ?? []);
        const counts = countResults(results);
        ...
        const apiId = roomCode(raw, prev, tableId);
        ...
        const next: DgTableSnapshot = {
          ...
          round: roundNum ?? prev?.round ?? 0, ...counts, results, beads, trend: prev?.trend ?? "", live: true, dealerPhoto, streamUrl: prev?.streamUrl,
  ...
  enterBridgeMode() {
    ...
    // Keep the ONE already-authenticated vendor WebSocket alive. The DG iframe
    // is proxied through a local WebSocket view of this same socket, so the
    // game and floating assistant share one upstream connection/session.
    this.foregroundBridgeActive = true;
    this.transportMode = "raw";
    this.foregroundBridgeSinks.clear();
    if (this.status === "connected") this.setStatus("connected", "DG 遊戲與懸浮輔助共用同一即時連線");
    else this.setStatus("connecting", "正在建立 DG 共用即時連線...");
    this.log("SharedSocket｜DG 遊戲與懸浮輔助共用同一 Vendor WebSocket");
  }
  ...
  async forwardForegroundFrame(data: Buffer, localReply?: (data: Buffer) => void) {
    ...
    try {
      ...
      if (!(cmd === 10086 || cmd === 45 || cmd === 2 || cmd === 5011 || cmd === 87 || cmd === 24 || cmd === 99)) {
        const now = Date.now();
        this.recentForegroundRequests.push({ at: now, cmd, data: Buffer.from(data) });
        this.recentForegroundRequests = this.recentForegroundRequests.filter(x => now - x.at <= 15_000).slice(-24);
      }
      if (cmd === 10086 || cmd === 45 || cmd === 2 || cmd === 5011 || cmd === 87 || cmd === 24 || cmd === 99) {
        if (localReply) {
  ...
  leaveBridgeMode() {
    ...
    this.log("SharedSocket｜已離開 DG 遊戲畫面；原即時連線繼續保留");
  }
